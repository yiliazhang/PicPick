...
- (void)showImage
{
    if (_photo.firstShow) { // 首次显示
        ...
        if (![_photo.url.absoluteString hasSuffix:@"gif"]) {
            ...
            [_imageView setImageWithURLRequest:request placeholderImage:_photo.placeholder success:^(NSURLRequest * _Nonnull request, NSHTTPURLResponse * _Nullable response, UIImage * _Nonnull image) {
                photo.image = image;
...
- (void)photoStartLoad
{
    if (_photo.image) {
        ...
        NSURLRequest *request = [NSURLRequest requestWithURL:_photo.url];
        [_imageView setImageWithURLRequest:request placeholderImage:_photo.srcImageView.image success:^(NSURLRequest * _Nonnull request, NSHTTPURLResponse * _Nullable response, UIImage * _Nonnull image) {
            loading.progress = 1;
        } failure:nil];
    }
}
